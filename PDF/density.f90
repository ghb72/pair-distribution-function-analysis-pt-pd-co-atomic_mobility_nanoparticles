...
real, dimension(nhis) :: hist,hs1,hs2,base,dens
character(32) :: a
character(2) :: ele
...
! Calculando la densidad usando la ecuación ρ(r) = 1/(4πNr²)∑ᵢⱼfᵢfⱼδ(r-rᵢ)
do i = 1,npart-1
   do j = i+1,npart
      if (i == j) cycle  ! Evitar auto-interacción
      bij = b(i)*b(j)
      dx = x(i)-x(j)
      dy = y(i)-y(j)
      dz = z(i)-z(j)
      ...
      if (r < box) then
         ig = int(r/delr-0.5)
         ! La función delta se aproxima distribuyendo en bins, normalizado por el volumen esférico
         hist(ig) = hist(ig) + 2.0*bij/(npart*r*r*delr)
         hint = hint + 1.0/r
         rmed = rmed + r
         if (r < rmin) rmin = r
         if (r > rmax) rmax = r
...
hist = hist/(4.0*pi*hint*npart*bm**2)  ! Normalizando por el volumen esférico

rmed = rmed/m

! Aplicando suavizado para obtener una distribución más continua
...
write(11,*) '#r(A)        ρ(r)         Base         ρ_suavizada(r)'
do i = 1,nhis
   r = i*delr
   write(11,30) r, hist(i), base(i), dens(i)
end do
