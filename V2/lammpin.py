...
def wlammpin(atpos,eleList,nameout):
    lin = open(nameout,'w')
    n = len(atpos)
    m = len(eleList)
...
#atpos, eleList = getStr(name)
atpos, eleList = leer_xyz(name)
#print(atpos)
#spherical_cut(atpos, 26, 'sphericShellPd.xyz')  # Realizar el corte esférico y escribir el nuevo archivo .xyz

#atpos = centerStr(atpos)
wlammpin(atpos,eleList,'coords_'+name[:-4]+'.ini')
shutil.move('coords_'+name[:-4]+'.ini','lammps\\coords_'+name[:-4]+'.ini')
#wXYZ(atpos)
