...
def decShell(ns:int,d:float,cut=False):
...
def rdodShell(ns:int,d:float,cut=False):
...
def tomakeshell_wxyz(coords:list,element:str,name:str):
	center = True
	previous = False
	m1 = 0
	prev = []
...
def sphericalShave(coords:list,r:float):
	cc=[]
	for p in coords:
