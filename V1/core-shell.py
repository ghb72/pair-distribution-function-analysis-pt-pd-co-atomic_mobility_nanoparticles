# EL PROGRAMA ESTA HASTA ABAJO, LO HE MODIFICADO MUCHO
...
def wxyz(coords,elems,non_rand_elem):

	#name = 'shell spheric.xyz'
	center = True
	previous = False
	m1 = 0
	prev = []
...
#		rotate(atpos,-0.25,'x')
	...
	n = m1 + m2
	xyzfile.write(str(n) + '\n\n')
	elem_c=ran.choice(elems)
	# si para el if siguiente ele2=ran.choice entonces el programa mete impurezas aleatoriamente en la nanopartícula
	if previous:
		for atom in atpos:
			ele2,x,y,z = atom
			#ele2=elem_c
			#ele2 = ran.choice(elems)
			xyzfile.write('{0:2s}{1:12.5f}{2:12.5f}{3:12.5f}\n'.format(ele2,x,y,z))
	for i in range(m2):
		#ele1 = ran.choice(elems)
		ele1=elem_c
		x,y,z = coords[i]
...
# VARIABLES: NOMBRE, d, ns, elems, opmode, etc. #################################
if __name__ == '__main__':
	elems=['Pt','Pd','Co']
	set_elem= 'Pd' #elems[2]
	#ns=4 # see: # ##### ITER
	d=2.75
    #ele = 'Al' # sys.argv[1] # now ele doesnt work, you must work with elems and set_elem
	name = 'shells\shellPd.xyz' # name of file to be created
opmode = 'c' #sys.argv[1] #opmode r: random elements, c: capas
# random elements (r) or by capas (c) set sys-arg[1] r or c, for this set_elem = elems[i]
capas = 35
#nns = sys.argv[2] # ns input
#ns = int(nns)
#d = 2 #float(sys.argv[3]) #d input
#d = 2.9

# EJECUCION DE FUNCIONES Y ESCRIBIR SHELL XYZ ###################################

if opmode == 'r':
	pass

if opmode == 'c':
	elems=[set_elem,set_elem,set_elem]

# ##### ITER
for ns in range(0,capas):
	if ns == 0: 
		coords = [np.array([0.0,0.0,0.0])]
	else:
		coords = cbocShell(ns,d)

	#coords = sphericalShave(coords,25)
	#coords = randomRemoval(coords,p)

	wxyz(coords,elems,set_elem)
