...
dict_elementos = {'Pt': 0.28, 'Pd': 0.45, 'Co': 0.27}  # Diccionario de nuevos elementos y sus probabilidades
atpos, eleList = leer_xyz(name) #lee el xyz

######################################################
#corta esféricamente
rad = ni # quitar despues (radio de corte)
name = 'shells\shellPd_'+str(rad)+'.xyz'
new_atpos,new_eleList = cambiar_elementos_prob(atpos, dict_elementos)
new_atpos = spherical_cut(new_atpos, rad, name)  # Realizar el corte esférico y escribir el nuevo archivo .xyz
...
#modify_md9(t_relax)
#odify_str_coords(ni) #lee el archivo coords.ini según el numero de prueba
...
dump='lammps\dump.xyz'
newdump='lammps\dump_'+str(ni)+'.xyz'
os.rename(dump,newdump)
#quitlines(dump) #modificar

##################################################
#cambiar etiquetas del archivo dump
dict_etiquetas = {1: 'Pt', 2: 'Pd', 3: 'Co'}  # Diccionario de etiquetas
cambiar_etiquetas_xyz(newdump, dict_etiquetas, newdump)

###########################################
#crea la pdf
newdump.replace('lammps\\', '')
print(newdump)
comando = ['python', '-u', 'pdf.py', newdump]
subprocess.run(comando)

#######################################33

# mueve la pdf a la carpeta Graphics
archivo = 'dump_'+str(ni)+'.dat'
origen = os.path.join('lammps', archivo)
destino = os.path.join('graphics', archivo)
shutil.move(origen, destino)
