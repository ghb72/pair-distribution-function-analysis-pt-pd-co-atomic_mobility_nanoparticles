...
def pol_3ab_rad_ch(atpos:list,eleList:list,percentes:dict,Diam:float, new_element:str, a:float, b:float):
    print(f'Cambia en potencia el porcentaje radial de átomos en {percentes}')
    new_atpos=[]
    per = percentes[new_element]
    p = (a/(per-b/3)) - 3
...
def remove_at_powrad(atpos:list,percent,maxrad:float,a = 1):
    # a = constant of proportionality
    print(f'Elimina aleatoriament el {percent}% de atpos para p(r)=a*x^(1/percent% -1)')
    new_atpos = []
    p = (a/percent)-1
    print(f'fracción quitada es {percent}, p es {p}')
    for atom in atpos:
        r = np.sqrt(atom[1]**2+atom[2]**2+atom[3]**2)
        x = pow(r/maxrad,3)
        y = a*pow(x,p)
...
def pol_ab_rad_remove(atpos:list,percent:float,maxrad:float, a=1, b=0):
    print(f'Quita el porcentaje radial de átomos en {percent}')
    new_atpos=[]
    p = a/(percent-b)-1
    print(f'per es {percent}, p es {p}')
    print(f'p(x)={a}*x^({a}/({percent}-{b}) -1) + {b}')
    for atom in atpos:
        r = np.sqrt(atom[1]**2+atom[2]**2+atom[3]**2)
        x = pow(r/maxrad,3)
        y = a*pow(x,p)+b
